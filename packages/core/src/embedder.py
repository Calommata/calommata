...
Hugging Face의 코드 임베딩 모델을 사용하여 코드를 벡터로 변환합니다.
"""
...
class CodeEmbedder(BaseModel):
    ...
    Hugging Face의 코드 전용 임베딩 모델을 사용하여
    코드 조각을 벡터 표현으로 변환합니다.

    지원하는 모델:
    - microsoft/codebert-base (기본값)
    ...
    model_name: str = Field(
        ...
        description="사용할 Hugging Face 임베딩 모델 이름",
    )

    model_kwargs: dict[str, Any] = Field(
        default_factory=lambda: {"device": "cpu"}, description="모델 초기화 파라미터"
    )

    encode_kwargs: dict[str, Any] = Field(
        default_factory=lambda: {"normalize_embeddings": True},
        description="인코딩 파라미터",
    )
